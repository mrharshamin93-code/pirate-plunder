...
const SKULL_TEXTURE: Texture2D = preload("res://assets/pirate-skull.svg")
...
func _bind_hero(found: Label) -> void:
	...
	if root == null:
		return
	if hero_title == found and sunk_root == root and is_instance_valid(left_skull) and is_instance_valid(right_skull):
		return

	hero_title = found
	sunk_root = root
	# Remove the platform-dependent Unicode skull glyphs. The SUNK text remains a normal label.
	hero_title.text = "SUNK!"

	var old_left: Node = sunk_root.get_node_or_null("HeroSkullLeft")
	if old_left != null:
		old_left.queue_free()
	var old_right: Node = sunk_root.get_node_or_null("HeroSkullRight")
	if old_right != null:
		old_right.queue_free()

	left_skull = _make_skull("HeroSkullLeft")
	right_skull = _make_skull("HeroSkullRight")
	sunk_root.add_child(left_skull)
	sunk_root.add_child(right_skull)
	left_skull.move_to_front()
	right_skull.move_to_front()

	if not sunk_root.resized.is_connected(_on_root_resized):
		sunk_root.resized.connect(_on_root_resized)
	if not sunk_root.visibility_changed.is_connected(_on_root_visibility_changed):
		sunk_root.visibility_changed.connect(_on_root_visibility_changed)
...
func _layout_skulls() -> void:
	...
	var icon_w: float = 52.0 * sx
	var icon_h: float = 41.0 * sx
	var gap: float = 10.0 * sx
