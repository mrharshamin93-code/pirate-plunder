...
func _build_overlay()->void:
 ...
 # Dynamic close-button feedback uses the exact artwork already baked into the page.
 var close_feedback:=TextureRect.new();var close_atlas:=AtlasTexture.new();close_atlas.atlas=bg.texture;close_atlas.region=Rect2(338,18,42,54);close_feedback.texture=close_atlas;close_feedback.position=Vector2(338,18);close_feedback.size=Vector2(42,54);close_feedback.expand_mode=TextureRect.EXPAND_IGNORE_SIZE;close_feedback.stretch_mode=TextureRect.STRETCH_SCALE;close_feedback.mouse_filter=Control.MOUSE_FILTER_IGNORE;overlay.add_child(close_feedback)
 var close_hit:=Button.new();close_hit.text="";close_hit.flat=true;close_hit.position=Vector2(338,18);close_hit.size=Vector2(42,54);close_hit.focus_mode=Control.FOCUS_NONE;close_hit.add_theme_stylebox_override("normal",_transparent_box());close_hit.add_theme_stylebox_override("hover",_transparent_box());close_hit.add_theme_stylebox_override("pressed",_transparent_box());close_hit.button_down.connect(func()->void:close_feedback.position=Vector2(340,21);close_feedback.size=Vector2(38,49);close_feedback.modulate=Color(0.82,0.82,0.82,1.0));close_hit.button_up.connect(func()->void:close_feedback.position=Vector2(338,18);close_feedback.size=Vector2(42,54);close_feedback.modulate=Color.WHITE);close_hit.mouse_exited.connect(func()->void:close_feedback.position=Vector2(338,18);close_feedback.size=Vector2(42,54);close_feedback.modulate=Color.WHITE);close_hit.pressed.connect(_close);overlay.add_child(close_hit);close_hit.move_to_front()
